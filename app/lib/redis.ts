...
export async function createUser(userData: Omit<User, 'id' | 'created_at' | 'updated_at'>): Promise<User> {
  ...
  await redis.hset(`user:${user.google_user_id}`, user as unknown as Record<string, string>);
...
export async function getUserByEmail(email: string): Promise<User | null> {
  const googleUserId = await redis.get(`users:by_email:${email}`);
...
export async function updateUser(googleUserId: string, updates: Partial<User>): Promise<User> {
  ...
  await redis.hset(`user:${googleUserId}`, updated as unknown as Record<string, string>);
...
export async function createDiscordChannel(
  ...
  await redis.hset(`discord_channel:${id}`, channel as unknown as Record<string, string>);
...
export async function listUserDiscordChannels(userId: string): Promise<DiscordChannel[]> {
  const channelIds = await redis.smembers(`user:${userId}:discord_channels`);
...
export async function createCalendarSubscription(
  ...
  await redis.hset(`calendar_subscription:${id}`, subscription as unknown as Record<string, string>);
  await redis.sadd(`user:${subscription.user_id}:calendar_subscriptions`, id);

  if (subscription.active) {
    await redis.sadd('subscriptions:active', id);
  }
...
export async function listUserSubscriptions(userId: string): Promise<CalendarSubscription[]> {
  const subscriptionIds = await redis.smembers(`user:${userId}:calendar_subscriptions`);
...
export async function findSubscriptionByGoogleChannel(googleChannelId: string): Promise<CalendarSubscription | null> {
  const subscriptionId = await redis.get(`google_channel:${googleChannelId}`);
...
export async function updateCalendarSubscription(
  ...
  await redis.hset(`calendar_subscription:${subscriptionId}`, updated as unknown as Record<string, string>);

  // Update active subscriptions set
  if (updates.active !== undefined) {
    if (updates.active) {
      await redis.sadd('subscriptions:active', subscriptionId);
    } else {
      await redis.srem('subscriptions:active', subscriptionId);
    }
  } else if (existing.active) {
    // Ensure it's in the set if it was active
    await redis.sadd('subscriptions:active', subscriptionId);
  }
...
export async function cacheEvent(
  ...
  await redis.hset(key, event as unknown as Record<string, string>);
...
export async function clearEventCache(subscriptionId: string): Promise<void> {
  const eventIds = await redis.smembers(`subscription:${subscriptionId}:events`);
...
// Helper to get all active subscriptions
export async function getAllActiveSubscriptions(): Promise<CalendarSubscription[]> {
  // This requires maintaining a set of all subscription IDs
  // For MVP, we'll maintain this set when creating/deleting subscriptions
  const subscriptionIds = await redis.smembers('subscriptions:active');
  if (!subscriptionIds || subscriptionIds.length === 0) return [];
  ...
  return subscriptions.filter((s): s is CalendarSubscription => s !== null && s.active);
}
...
export async function getSubscriptionsExpiringSoon(hoursAhead: number = 6): Promise<CalendarSubscription[]> {
  const allActive = await getAllActiveSubscriptions();
  const now = Date.now();
  const threshold = now + (hoursAhead * 60 * 60 * 1000);

  return allActive.filter(sub => {
    if (!sub.google_channel_expiration) return false;
    // Renew if expiring within threshold OR already expired
    return sub.google_channel_expiration <= threshold;
  });
}
